...
import { initProgressStatus } from '@redux/sagas/player.saga'
...
class MusicPlayer extends Component<IProps, any> {
  ...
  renderMusicPlayer = () => {
    ...
    const sourceUrl = getMusicLink(track.id)
    const title = get(track, 'name', '')
    const artist = get(track, 'ar', [])
    // const title = track.name ? track.name : ''
    // const artist = track.ar ? track.ar : []
    let author = ''
    artist.forEach((item: IArtist, index: number) => {
    ...
    return (
      ...
      >
        {/* 播放器 */}
        { initStatus ? <Audio source={sourceUrl} /> : null }
        {/* 背景渐变图片 */}
