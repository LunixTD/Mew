...
function* loginStart(action: any) {
  ...
  if (type === 'mobile') {
    ...
    yield call(AsyncStorage.setItem, 'profile', JSON.stringify(res.profile))
    // console.log(res)
  ...
  switch(code) {
    ...
    case 501:
    case 502:
      yield call(ToastAndroid.show, '用户名或密码错误', 1000)
      break
    case 200:
      yield call(ToastAndroid.show, '登录成功', 1000)
...
function* logoutStart() {
  ...
  if (res.code === 200) {
    ...
    yield call(ToastAndroid.show, '登出成功', 1000)
    yield call(showAuthPage, true)
  } else {
    yield call(ToastAndroid.show, '登出失败,请重试', 2000)
    console.log('登出失败')
    console.log(res)
  }
}
